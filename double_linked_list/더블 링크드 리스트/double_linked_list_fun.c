...
void dll_append_node(node** head, node* new_node)
{
	...
	if ((*head) == NULL)// 링크드 리스트와 동일
	{
		*head = new_node;
		...
		new_node->prev_node = tail;// 마지막 새노드 앞에 꼬리와 연결해줌
	}
}
...
void cdll_append_node(node** head, node* new_node)
{
	...
	else
	{
		...
		tail->next_node->prev_node = new_node; //헤드의 이전 노드(이전 마지막 노드)에 새로운 노드와 연결
		tail->next_node = new_node;// 이전 마지막 노드의 다음 노드를 새로운 노드 참조
